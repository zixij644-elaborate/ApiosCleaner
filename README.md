...
*á¼„Ï€Î¹Î¿Ï‚ (Ã¡pios) â€?ancient Greek for "pear"*
...
independently â€?see [Beyond a straight port](CHANGELOG.md#beyond-a-straight-port).

> âš ï¸ **Status**: v0.1.0 â€?first release. macOS adapter works (CLI); Linux/Windows
> adapters compile with default XDG behavior; the GUI is planned.

## Why this project

- **Universal binary thinning (`lipo`)** â€?a signature Pearcleaner feature
...
| Area | State |
|---|---|
| Core engine (scan/match/orphan/trash) | âœ?implemented + unit tested |
| CLI (`list` / `uninstall` / `orphan` / `clean-orphan` / `dev-clean` / `pkg` / `lipo`) | âœ?works on macOS, output verified against the reference implementation |
| Platform adapters | âš ï¸ macOS: paths / app metadata / trash in place; Linux, Windows: â¬?planned |
| Verification | âœ?9/9 and 17/17 file sets identical on test apps |
| UI | â¬?planned |

## Install

**Pre-built binary** (recommended) â€?download the latest release from
...
> âš ï¸ macOS Gatekeeper: the binary is ad-hoc signed; the first run from a
> downloaded zip may need right-click â†?**Open**, or
...
> âš ï¸ Deleting commands move files to the Trash and ask for confirmation; they
> never permanently delete. Run the binary without `sudo` â€?the critical-path
...
- [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) â€?the portable-core + adapter
